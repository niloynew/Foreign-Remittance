...
public class NostroAccountServiceImpl implements NostroAccountService {
  ...
  public PagedResult<NostroAccount> getAccounts(Pageable pageable) {
    Page<NostroAccountEntity> pagedNostroAccountEntities = repository.findAll(pageable);
    return PagedResultBuilder.build(pagedNostroAccountEntities, mapper.entityToDomain());
  }
