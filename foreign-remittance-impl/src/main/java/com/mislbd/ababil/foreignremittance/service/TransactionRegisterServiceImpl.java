...
public class TransactionRegisterServiceImpl implements TransactionRegisterService{
    ...
    public void doRegister(List<CbsTransaction> transactions, Long remittanceTransactionId) {
        ...
        if(transactions != null && !transactions.isEmpty() && remittanceTransactionId != null){
            transactions.forEach(txn -> {
                        TransactionRegisterEntity registerEntity = transactionRegisterMapper.cbsDomainEntity().map(txn);
    ...
    public List<TransactionRegister> findTransactionRegisterByGlobalTxnNumber(Long globalTxnNumber) {
        return ListResultBuilder.build(transactionRegisterRepository.findAllByVoucherNumber(globalTxnNumber), transactionRegisterMapper.entityToDomain());
    }

    @Override
    public void invalidRegister(Long globalTxnNumber) {
        List<TransactionRegisterEntity> registers = transactionRegisterRepository.findAllByVoucherNumber(globalTxnNumber);
        registers = registers.stream()
                .map(register -> register.setValid(false))
                .collect(Collectors.toList());
        transactionRegisterRepository.saveAll(registers);
    }

    @Override
    public List<TransactionRegister> findTransactionRegisterByRemittanceTransaction(Long remittanceTransactionId) {
        return ListResultBuilder.build(transactionRegisterRepository.findAllByRemittanceTransactionId(remittanceTransactionId), transactionRegisterMapper.entityToDomain());
    }
}
